
<?php
/*
Template Name: Resources Listing
*/


get_header();

global $post;

$showOnThisPage = get_field('show_on_this_page', $post->ID);
// Get custom_page_sidebar meta data
$menu_items = isset($custom_page_sidebar['menu_items']) ? $custom_page_sidebar['menu_items'] : array();
$header_text = get_field('header_block_text', 'option');
?>
<section id="primary" class="resources-listing-page">

  <div class="container">
    <div class="row">
      <div class="col-12 main-area title-area">
          <h1><?php echo get_the_title(); ?></h1>
          <div class="resources-header-text">
             <?php if ($header_text) : ?>
              <p><?php echo esc_html($header_text); ?></p>
            <?php endif; ?>
          </div>
           </div>
    </div>
  </div>

  <div class="container holder">
    <div class="row">
      <?php if( have_rows('page_builder') ): ?>
        <div class="col-md-12 main-area">
          <div class="resources-filter-section">
            <div class="top-filter-section">
              <?php echo do_shortcode('[facetwp facet="resource_media_resource_topic"]'); ?>
            </div>
            <div class="bottom-filter-section">
              <button class="btn-toggle-filter" type="button" aria-label="Toggle advanced search and filter">
                <div class="button-text">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <path d="M6.17 18C6.3766 17.4145 6.75974 16.9074 7.2666 16.5488C7.77346 16.1902 8.37909 15.9976 9 15.9976C9.62091 15.9976 10.2265 16.1902 10.7334 16.5488C11.2403 16.9074 11.6234 17.4145 11.83 18H22V20H11.83C11.6234 20.5855 11.2403 21.0926 10.7334 21.4512C10.2265 21.8099 9.62091 22.0025 9 22.0025C8.37909 22.0025 7.77346 21.8099 7.2666 21.4512C6.75974 21.0926 6.3766 20.5855 6.17 20H2V18H6.17ZM12.17 11C12.3766 10.4145 12.7597 9.90744 13.2666 9.5488C13.7735 9.19015 14.3791 8.99756 15 8.99756C15.6209 8.99756 16.2265 9.19015 16.7334 9.5488C17.2403 9.90744 17.6234 10.4145 17.83 11H22V13H17.83C17.6234 13.5855 17.2403 14.0926 16.7334 14.4512C16.2265 14.8099 15.6209 15.0025 15 15.0025C14.3791 15.0025 13.7735 14.8099 13.2666 14.4512C12.7597 14.0926 12.3766 13.5855 12.17 13H2V11H12.17ZM6.17 4.00001C6.3766 3.41448 6.75974 2.90744 7.2666 2.5488C7.77346 2.19015 8.37909 1.99756 9 1.99756C9.62091 1.99756 10.2265 2.19015 10.7334 2.5488C11.2403 2.90744 11.6234 3.41448 11.83 4.00001H22V6.00001H11.83C11.6234 6.58554 11.2403 7.09258 10.7334 7.45122C10.2265 7.80986 9.62091 8.00246 9 8.00246C8.37909 8.00246 7.77346 7.80986 7.2666 7.45122C6.75974 7.09258 6.3766 6.58554 6.17 6.00001H2V4.00001H6.17ZM9 6.00001C9.26522 6.00001 9.51957 5.89465 9.70711 5.70712C9.89464 5.51958 10 5.26523 10 5.00001C10 4.73479 9.89464 4.48044 9.70711 4.2929C9.51957 4.10537 9.26522 4.00001 9 4.00001C8.73478 4.00001 8.48043 4.10537 8.29289 4.2929C8.10536 4.48044 8 4.73479 8 5.00001C8 5.26523 8.10536 5.51958 8.29289 5.70712C8.48043 5.89465 8.73478 6.00001 9 6.00001ZM15 13C15.2652 13 15.5196 12.8947 15.7071 12.7071C15.8946 12.5196 16 12.2652 16 12C16 11.7348 15.8946 11.4804 15.7071 11.2929C15.5196 11.1054 15.2652 11 15 11C14.7348 11 14.4804 11.1054 14.2929 11.2929C14.1054 11.4804 14 11.7348 14 12C14 12.2652 14.1054 12.5196 14.2929 12.7071C14.4804 12.8947 14.7348 13 15 13ZM9 20C9.26522 20 9.51957 19.8947 9.70711 19.7071C9.89464 19.5196 10 19.2652 10 19C10 18.7348 9.89464 18.4804 9.70711 18.2929C9.51957 18.1054 9.26522 18 9 18C8.73478 18 8.48043 18.1054 8.29289 18.2929C8.10536 18.4804 8 18.7348 8 19C8 19.2652 8.10536 19.5196 8.29289 19.7071C8.48043 19.8947 8.73478 20 9 20Z" fill="#3D3D3D"/>
                    </svg> 
                    <span>Advanced search and filter</span>
                  </div>
              
                  <div class="button-status">
                    <span class="button-status-text">Open</span>
                    <span class="button-status-icon"></span>
                  </div>
              </button>
              <div class="advanced-search-container">
                <div class="advanced-search-wrapper">
                 <div class="advanced-search-inner">
                    <label class="search-field"> Search
                      <?php echo do_shortcode('[facetwp facet="resource_media_search"]'); ?>
                    </label>
                    <label class="select-dropdown"> Audience
                      <?php echo do_shortcode('[facetwp facet="resource_media_audience"]'); ?>
                    </label>
                    <label class="select-dropdown"> Service
                      <?php echo do_shortcode('[facetwp facet="resource_media_service"]'); ?>
                    </label>
                    <label class="select-dropdown"> Content type
                      <?php echo do_shortcode('[facetwp facet="resource_media_content_type"]'); ?>
                    </label>
    
                    <button class="btn-submit" onclick="FWP.refresh()">Search and filter</button>
                 </div>
                </div>
                <div class="selection-reset-filter-wrapper">
                  <div class="facetwp-selections">
                  </div>
                  <div class="reset-filters">
                    <?php echo do_shortcode('[facetwp facet="resource_media_reset"]'); ?>
                  </div>
                </div>
              </div>
            </div>
            <div class="member-login-cta-wrapper">
              <?php if (!isset($_COOKIE['lgi'])) : ?>
               <div class="member-login-cta-text">
                  <h2>Are you a member?</h2>
                  <p>Login to see member exclusive content. <a href="/join-us/our-members/">Click here</a> to find out if you’re a member.</p>
               </div>
                <div class="member-login-cta">
                  <a href="/login" class="btn btn-primary">Log in</a>
                </div>
              <?php else : ?>
               <div class="member-login-cta-text">
                  <h2>Welcome back!</h2>
                  <p>You are logged in - browse resources to see member exclusive content.</p>
               </div>
              <?php endif; ?>
            </div>
          </div>
          <div class="listing-area">
           <div class="sorting-area">
              <div class="show-members-only">
                <div class="label"> Show member exclusive content </div>
                <?php echo do_shortcode('[facetwp facet="resource_media_members_only"]'); ?>
              </div>
              <div class="sort-by-wrapper">
                <div class="label">Sort by:</div>
                 <?php echo do_shortcode('[facetwp facet="resource_media_sort_by"]'); ?>
              </div>
           </div>

            <?php echo do_shortcode('[facetwp template="resource_media"]'); ?>
          </div>
          <div class="pagination-area">
            <?php echo do_shortcode('[facetwp facet="resource_media_pagination"]'); ?>
          </div>
        </div>
      <?php endif; ?>

    </div>
  </div>
</section>

<?php
get_footer();
?>


<style>
  .facet-dropdown-toggle {
    margin-bottom: 6px;
    padding: 6px 12px;
    background: #f2f2f2;
    border: 1px solid #ccc;
    cursor: pointer;
  }

  .facet-dropdown-wrapper {
    margin-bottom: 1em;
    position:absolute;
    background-color: #fff;
  }

</style>